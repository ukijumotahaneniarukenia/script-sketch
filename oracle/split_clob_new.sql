...
with sub as(select cast(collect(args_map(grp_key,tgt)) as args_map_liz) as nnn from test_tbl)select * from sub,table(split_clob(nnn,:split_chank_size));

--create or replace function split_clob(rsv_args args_map_liz,split_chank_size number)
--return rt_map_liz
--as
--rt rt_map_liz;
--begin
