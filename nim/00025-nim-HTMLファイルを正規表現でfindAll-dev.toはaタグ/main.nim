  ...
  for filename in filenames:
    block:
      ...
      defer : #go-langの遅延実行と同じ。try-catchのfinallyと同じ。
        close(wf)
        close(rf)
      rt_ary.add(rf.readAll().findAll(re"<a.*?a>")) #要素なしSeqは追加されないことがわかった便利。 #複数行にわたってマッチさせたいので、一括読込。正規表現のところ変数にしたんだよな。
      #rt_ary = rt_ary.mapIt(it.replacef(re"<a(.*)>","$1")).filterIt(it != "")
    for e in rt_ary:
      echo e
      #wf.writeLine("[")
