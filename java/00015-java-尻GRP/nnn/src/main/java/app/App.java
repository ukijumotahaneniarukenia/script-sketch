...
public class App {
    public static void main( String[] args ) {
        ...
        Map<String, List<String>> rt = shirigrp(l,1);
        System.out.println(rt);
    }

    private static Map<String, List<String>> shirigrp (List<String> s,Integer n){
        if(n > s.stream().min(Comparator.comparing(e -> e.length())).get().length()){
            return new HashMap<>();
        }else{
            return s.stream().collect(Collectors.groupingBy(e->e.substring(e.length() - n)
