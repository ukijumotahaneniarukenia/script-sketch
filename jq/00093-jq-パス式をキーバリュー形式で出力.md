...
CMD
```
$ cat a.json  | jq -cr '. as $in|[path(..)]|map(. as $path|{"k":($path|map(if type == "string" then "."+tojson else "["+tostring+"]" end)|join("")),"v":($in|getpath($path))})[]|"\(.k)\t\(.v|tojson)"'|sed '1ikey\tvalue'
```
