...
for keyword in ${KEYWORDS[@]} ;do
  ...
  eval echo {$START_HOST_NO..$END_HOST_NO} | xargs -n1|nl|\
    while read n h;do
      echo machinectl shell root@$REPLICA_NAME-$(printf "%02d" $[n+1]) /usr/bin/cp $WORKDIR/$(ls $keyword* | grep $SEGMENT_THIRD_OCTET_IP.$[$SEGMENT_FORTH_OCTET_START_IP+$n]) $DEPLOY_DIR/$keyword;
    done
